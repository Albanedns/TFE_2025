...
df = pd.read_csv('errors_with_age.csv')
...
for _, row in combinations.iterrows():
    ...
    subset= subset.dropna(subset=['distance_error', 'AgeGroup', 'APOE', 'Gender', 'VideoGame', 'Education', 'Participant', 'VersionAppleGame'])
     
    
    #Standardisation   
    scaler = StandardScaler()
    subset = subset[['distance_error', 'AgeGroup', 'APOE', 'Gender', 'VideoGame', 'Education', 'Participant', 'VersionAppleGame']]
    ...
    if subset.shape[0] > 1:
        ...
        try:
            ...
            mod_lmer <- lmer(distance_error ~ AgeGroup * APOE + Gender + VideoGame + Education + (1|Participant) + (1|VersionAppleGame), data=df)
            ...
            plot <- emmip(mod_lmer, APOE ~ AgeGroup, CIs = TRUE, type = "response",
                                ...
                                xlab = "Age (years)", ylab = " Distance Error (v m) ") +
                    ggtitle("PPI: Distance Error") +
                    theme_bw() +
            ...
            #plot <- plot + 
                #annotate("segment", x = 2.9, xend = 3.1, y = 2, yend = 2,linewidth = 0.5) +  # segment horizontal
                #annotate("segment", x = 2.9, xend = 2.9, y = 2+0.01, yend = 2-0.01,linewidth = 0.5) +
                #annotate("segment", x = 3.1, xend = 3.1, y = 2+0.01, yend = 2-0.01,linewidth = 0.5) +
                #annotate("text", x = 3, y = 2.01, label = "*", size = 7)

            ggsave(filename = "Plot/distance_error_PPI.png", plot = plot, width = 10, height = 6)
            ''')
